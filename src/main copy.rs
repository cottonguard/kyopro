...
fn run<I: Input, O: Write>(mut kin: I, mut out: O) {
    let (a, b, c, d): (i64, i64, i64, i64) = kin.input();
    ...
    if det == 0 {
        ...
        let a = gcd(a, c);
        let b = gcd(b, d);
        for p in kin.iter::<(i64, i64)>().take(n) {
            let (x, y) = p;
            if ss {
                set.insert((x - y * a / b, y.rem_euclid(b)));
            } else {
                set.insert((x.rem_euclid(a), y - x * b / a));
            }
        }
    } else {
        let f = |(x, y)| {
            // (a * (d * x - c * y) / det, d * (-b * x + a * y) / det)
            (d * x - c * y, -b * x + a * y)
        };
        for p in kin.iter::<(i64, i64)>().take(n) {
            let (x, y) = f(p);
            set.insert((x.rem_euclid(det), y.rem_euclid(det)));
        }
    }
    outln!(out, "{}", set.len());
}
